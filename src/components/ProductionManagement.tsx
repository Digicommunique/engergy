...
import { Battery, Play, BarChart3, QrCode, ClipboardCheck, ArrowRight, Settings2, History, CheckCircle2, AlertOctagon } from 'lucide-react';
...
export default function ProductionManagement() {
  const [plans, setPlans] = useState<ProductionPlan[]>([
    { id: 'PP-44021', model: 'INV-150', quantity: 200, status: 'Completed', startDate: '2026-04-20', serialsGenerated: true },
  ...
  const [newPlan, setNewPlan] = useState({ model: 'INV-150', quantity: 100 });
  ...
  const handleGenerateSerials = (id: string) => {
    ...
    setTimeout(() => {
      setPlans(prev => prev.map(p => p.id === id ? { ...p, serialsGenerated: true, status: 'In Progress' } : p));
